...
(function () {
  ...
  function addStyles() {
    ...
    s.textContent = `
      /* Reliable, single-source mobile navigation. */
      @media (max-width:900px){
        html.ti-nav-lock, body.ti-nav-lock{overflow:hidden!important;overscroll-behavior:none!important}
        .sidebar#sidebar{
          position:fixed!important;left:0!important;top:0!important;
          width:min(300px,84vw)!important;height:100dvh!important;max-height:100dvh!important;
          margin:0!important;padding:84px 12px 24px!important;
          ...
          box-shadow:16px 0 42px rgba(27,39,75,.22)!important;
          transition:transform .24s cubic-bezier(.2,.8,.2,1),visibility 0s linear .24s!important;
          overflow-x:hidden!important;overflow-y:auto!important;
          -webkit-overflow-scrolling:touch!important;
        }
        .sidebar#sidebar.ti-drawer-visible{
          transform:translate3d(0,0,0)!important;
          visibility:visible!important;
          transition:transform .24s cubic-bezier(.2,.8,.2,1),visibility 0s linear 0s!important;
        }
        .ti-drawer-backdrop{
          position:fixed!important;inset:0!important;
          background:rgba(20,28,55,.28)!important;
          z-index:10000!important;display:none!important;opacity:0!important;
        ...
        .hamburger#hamburgerBtn{
          ...
          width:68px!important;height:58px!important;min-width:68px!important;
        ...
        #sidebar .sidebar-category-title,#sidebar h3,#sidebar h4,
        #sidebar .category-title,#sidebar [data-category-title]{
          font-weight:800!important;color:#17213f!important;
          letter-spacing:.01em!important;
        }
        #sidebar ul,#sidebar li{max-width:100%!important}
        #sidebar a,#sidebar button{touch-action:manipulation!important}
      }
      @media (min-width:901px){.ti-drawer-backdrop{display:none!important}}

      /* Consent popup. */
      .ti-consent{position:fixed;left:16px;right:16px;bottom:16px;z-index:20000;max-width:720px;margin:auto;padding:18px 20px;border:1px solid rgba(82,73,180,.18);border-radius:20px;background:rgba(255,255,255,.98);box-shadow:0 22px 70px rgba(30,39,78,.22);font-family:Inter,system-ui,sans-serif;color:#26324f}
  ...
  function installDrawer() {
    ...
    /* Remove competing inline handlers from the hamburger. */
    ...
    function close() {
      ...
      document.documentElement.classList.remove('ti-nav-lock');
      document.body.classList.remove('ti-nav-lock');
    ...
    function open() {
      ...
      document.documentElement.classList.add('ti-nav-lock');
      document.body.classList.add('ti-nav-lock');
    ...
    /* A delegated capture listener survives other scripts re-binding the button. */
    if (!window.__toolideaNavDelegated) {
      ...
      document.addEventListener('click', function (e) {
        ...
        if (closeTarget) { e.preventDefault(); close(); }
      }, true);
      document.addEventListener('pointerup', function (e) {
        var target = e.target && e.target.closest ? e.target.closest('#hamburgerBtn') : null;
        if (target && !target.__tiClickHandled) {
          /* Click normally follows pointerup; do not toggle twice. */
        }
      }, true);
      document.addEventListener('keydown', function (e) { if (e.key === 'Escape') close(); });
      window.addEventListener('hashchange', close);
      window.addEventListener('resize', function () { if (window.innerWidth > 900) close(); }, { passive: true });
      window.addEventListener('pageshow', function () { if (window.innerWidth <= 900) close(); }, { passive: true });
    }

    if (!sidebar.__tiCloseHandler) {
      sidebar.__tiCloseHandler = true;
      sidebar.addEventListener('click', function (e) {
        var item = e.target && e.target.closest ? e.target.closest('a,button,li') : null;
        if (item && !item.closest('.sidebar-category-title')) setTimeout(close, 80);
      });
    }

    /* Make category headings visually distinct even when generated by app.js. */
    function boldCategories() {
      sidebar.querySelectorAll('h2,h3,h4,.category-title,.sidebar-category-title,[data-category-title]').forEach(function (el) {
        el.style.fontWeight = '800';
      });
    }
    boldCategories();
    if (!sidebar.__tiObserver) {
      sidebar.__tiObserver = new MutationObserver(boldCategories);
      sidebar.__tiObserver.observe(sidebar, { childList: true, subtree: true });
    }
  }
  ...
  function installConsent() {
    ...
    box.id = 'toolideaConsent'; box.className = 'ti-consent'; box.setAttribute('role','dialog');
    ...
    box.addEventListener('click', function (e) {
      var action = e.target.closest('[data-consent]'); if (!action) return;
      var choice = action.dataset.consent;
      if (choice === 'privacy') {
        var modal = document.getElementById('modalOverlay'), modalBox = document.getElementById('modalBox');
