...
ready(()=>{
  ...
  function showHome(){
    ...
    document.querySelectorAll('.ad-incontent').forEach(x=>x.hidden=true);body.classList.add('home-view');body.classList.remove('tool-view');document.title='Toolinger — 90+ Free Online Tools';
  }
  function showTool(){
    ...
    document.querySelectorAll('.ad-incontent').forEach(x=>x.hidden=false);body.classList.remove('home-view');body.classList.add('tool-view');window.__toolingerCloseMenu?.();
  }
  if(!explicitTool){
    if(location.hash)history.replaceState(null,'',location.pathname+location.search);
    showHome();
    /* app.js may schedule a default route on its own startup; keep the first visit on landing. */
  ...
  $('#exploreToolsBtn')?.addEventListener('click',()=>$('#hamburgerBtn')?.click());
  ...
  const consentKey='toolinger-consent-v1',banner=$('#cookieBanner');let consent=null;try{consent=localStorage.getItem(consentKey)}catch(e){}if(!consent&&banner)banner.classList.add('show');
  const saveChoice=v=>{try{localStorage.setItem(consentKey,v)}catch(e){}banner?.classList.remove('show');};
  $('#cookieAcceptBtn')?.addEventListener('click',()=>saveChoice('all'));$('#cookieDeclineBtn')?.addEventListener('click',()=>saveChoice('necessary'));

  document.addEventListener('click',e=>{const b=e.target.closest('[data-modal]');if(!b)return;const key=b.getAttribute('data-modal'),overlay=$('#modalOverlay'),box=$('#modalBox');if(!overlay||!box||!window.POLICIES||!POLICIES[key])return;e.preventDefault();box.innerHTML='<button class="modal-close" type="button" data-close-modal>×</button>'+POLICIES[key];overlay.classList.add('open');});
